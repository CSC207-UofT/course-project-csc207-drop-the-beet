...
public class ImportantGateway {
    ...
    public static void writeAllImportant(UserManager user) {
        ...
        try {
            ...
            List<Schedule> importantWrite = user.getImportant().getSchedules();
            for (Schedule important : importantWrite) {
                jdbcsQlite.createImportantTaskByUserName(user.getName(), important.getTask(), important.getStart(), important.getEnd());
            }
